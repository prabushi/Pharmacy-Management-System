/*
 * Class to get the monthly revenue of this year
 */
package AdminGUI;
 ...
 * @author Prabushi
 */
public class MonthlyRevenue extends JPanel {

    float[] revenue = new float[12];
    public static MonthlyRevenue instance = null;
    JLabel Ltotal;
    JTextField total;
    JButton export;
    int month;
    ...
    public MonthlyRevenue() {
        ...
        try {
            ...
            for (int i = 0; i < 12; i++) {
                tot += revenue[i];
                barChartData.setValue(revenue[i], "Total Revenue", Integer.toString(i + 1));
            }
            total.setText(Float.toString(tot));
            JFreeChart barChart = ChartFactory.createBarChart("Monthly Revenue", "Month", "Revenue(Rs.)", barChartData, PlotOrientation.HORIZONTAL, false, false, false);
            ...
            export.addActionListener(new java.awt.event.ActionListener() {
                ...
                private void oneActionPerformed(ActionEvent evt) {
                    ...
                    for (int i = 0; i < 12; i++) {
                        content += " Month : " + monthName[i] + "     Total : " + Float.toString(revenue[i]) + "\n";
                        content += "\n" + "\n";
                    }
                    content += "\n" + "\n" + "Annual Revenue : " + total.getText();

                    writeToFile(content, Integer.toString(year) + "_Revenue.doc");
                }
            });
    ...
    public static MonthlyRevenue getInstance() {
        if (instance == null) {
            instance = new MonthlyRevenue();
        }
        return instance;
    }
}
